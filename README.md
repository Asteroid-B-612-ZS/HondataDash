...
自定义 Android 车载仪表盘，替代 Hondata 官方 App，运行在 800×480 横屏车机上。
通过蓝牙 SPP 连接 Hondata FlashPro，实时显示 42 个传感器参数。
...
### 6-LED 转速灯条 (ShiftLightView)

- 6 颗大 LED，RPM 渐进点亮
- 阈值: 4000 / 4500 / 5000 / 5500 / 6000 / 6200
- 颜色: 绿 绿 黄 黄 红 红
- ≥6400 RPM 全亮 1Hz 闪烁 (红色)
...
├── MainActivity.java          # 主界面, 数据绑定, 温度/AF闪烁控制
├── ScaleBarView.java          # 刻度进度条 (支持区间放大)
├── ShiftLightView.java        # 6-LED转速灯条
└── data/
    ├── DataSource.java        # 数据源接口 (Callback)
    ├── BluetoothSource.java   # 蓝牙SPP (FlashPro, 无握手弹窗)
    ├── HondataProtocol.java   # Hondata协议解析+缩放公式
